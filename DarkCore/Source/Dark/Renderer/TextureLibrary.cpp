...
namespace Dark
{
	...
	void SpritesheetLibrary::AddSpritesheet(std::string texSource, bool alpha, std::string name, unsigned int width, unsigned int height, unsigned int spriteWidth, unsigned int spriteHeight)
	{
		std::shared_ptr<Texture> texture = std::make_shared<Texture>(texSource, alpha);
		std::shared_ptr<SpriteSheet> spritesheet = std::make_shared<SpriteSheet>(texture, width, height, spriteWidth, spriteHeight);

		m_spritesheets[name] = spritesheet;
	}
	void SpritesheetLibrary::AddSpritesheet(const std::shared_ptr<Texture>& texture, std::string name, unsigned int width, unsigned int height, unsigned int spriteWidth, unsigned int spriteHeight)
	{
		std::shared_ptr<SpriteSheet> spritesheet = std::make_shared<SpriteSheet>(texture, width, height, spriteWidth, spriteHeight);
		m_spritesheets[name] = spritesheet;
	}
	void SpritesheetLibrary::AddSpritesheet(unsigned int width, unsigned int height, void* data, std::string name, unsigned int spriteWidth, unsigned int spriteHeight)
	{
		std::shared_ptr<Texture> texture = std::make_shared<Texture>(width, height, data);
		std::shared_ptr<SpriteSheet> spritesheet = std::make_shared<SpriteSheet>(texture, width, height, spriteWidth, spriteHeight);
